...
(function() {
  angular.module('impac.components.kpi', []).directive('impacKpi', ["$log", "$timeout", "$modal", "$templateCache", "ImpacKpisSvc", function($log, $timeout, $modal, $templateCache, ImpacKpisSvc) {
    return {
      restrict: 'EA',
      scope: {
        onDelete: '&',
      ...
      controller: ["$scope", function($scope) {
        ...
        if (!$scope.kpi["static"]) {
          ImpacKpisSvc.show($scope.kpi).then(function(renderedKpi) {
            ...
            if (!_.isEmpty($scope.getKpiTargets())) {
              ...
              return $scope.kpi.limit.value = _.values($scope.getKpiTargets()[0])[0];
            } else {
              $scope.kpi.limit = {
        ...
        $scope.cancelUpdateSettings = function() {
          if (!$scope.hasValidTarget()) {
            $scope.deleteKpi();
          }
          return $timeout(function() {
        ...
        $scope.getKpiUnit = function() {
          return ($scope.kpi.data != null) && $scope.kpi.data[$scope.kpi.element_watched].unit;
        };
        $scope.getKpiValue = function() {
          return ($scope.kpi.data != null) && $scope.kpi.data[$scope.kpi.element_watched].value;
        };
        return $scope.getKpiTargets = function() {
          return ($scope.kpi.targets != null) && $scope.kpi.targets[$scope.kpi.element_watched];
        };
      }]
    };
  }]);
    ...
    return {
      ...
      controller: ["$scope", "$timeout", "$log", function($scope, $timeout, $log) {
        ...
        ImpacKpisSvc.load().then(function() {
          return $scope.availableKpis = _.select(ImpacKpisSvc.getKpisTemplates(), function(k) {
            return _.isEmpty(k.attachables);
          });
        });
  ...
  angular.module('impac.services.events', []).service('ImpacEvents', ["$log", function($log) {
    ...
    this.notifyCallbacks = function(event, notification) {
      ...
      if (!observableCallbacks[event]) {
        ...
        ref = observableCallbacks[event];
        results = [];
        for (i = 0, len = ref.length; i < len; i++) {
          cb = ref[i];
          results.push(cb(notification));
        }
        return results;
      }
    };
...
(function() {
  angular.module('impac.services.kpis', []).service('ImpacKpisSvc', ["$log", "$http", "$filter", "$q", "$timeout", "ImpacRoutes", "ImpacMainSvc", "ImpacDashboardsSvc", "ImpacDeveloper", "ImpacAlerts", "ImpacEvents", "IMPAC_EVENTS", "ImpacUtilities", function($log, $http, $filter, $q, $timeout, ImpacRoutes, ImpacMainSvc, ImpacDashboardsSvc, ImpacDeveloper, ImpacAlerts, ImpacEvents, IMPAC_EVENTS, ImpacUtilities) {
    ...
    this.getAttachableKpis = function(widgetEngine) {
      return _self.load().then(function() {
        return _.select(_self.getKpisTemplates(), function(kpiTemplate) {
    ...
    this.load = function(force) {
      ...
      if (!_self.locked) {
        _self.locked = true;
        return ImpacDashboardsSvc.load(force).then(function() {
    ...
    this.refreshAll = function() {
      return _self.load().then(function() {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          k = ref[i];
          results.push(_self.show(k).then(function(renderedKpi) {}, function(errorResponse) {
            return $log.error("Unable to refresh all Kpis: " + errorResponse);
          }));
        }
        return results;
    ...
    this.validateKpiTarget = function(kpi) {
      if (!(kpi.limit && kpi.limit.value && kpi.limit.mode)) {
        return false;
      }
      return !(_.isEmpty(kpi.limit.value || _.isEmpty(kpi.limit.mode)));
    };
    ...
    this.formatKpiTarget = function(target, unit, mappings) {
      var label, targetMode;
      if (mappings == null) {
        mappings = [];
      }
      targetMode = _.keys(target)[0];
      label = _.find(mappings, function(map) {
        return map.mode === targetMode;
      }).label;
      return label + " " + ($filter('mnoCurrency')(target[targetMode], unit, false));
    };
    ...
    this["delete"] = function(kpi) {
      var url;
      return _self.load().then(url = ImpacRoutes.kpis["delete"](_self.getCurrentDashboard().id, kpi.id), $http["delete"](url).then(function(res) {
        ImpacEvents.notifyCallbacks(IMPAC_EVENTS.addOrRemoveAlerts);
        return res;
      }, function(err) {
  ...
  angular.module('impac.services.linking', []).provider('ImpacLinking', function() {
    ...
    provider.linkData = function(configData) {
      ...
      for (key in optional_links) {
        ...
        link = configData[key];
        if (link == null) {
          results.push(console.error("impac-angular linking.svc: No " + key + " is configured, Alerts are disabled."));
        } else {
          results.push(optional_links[key] = link);
        }
      }
      return results;
    };
...
(function() {
  angular.module('impac.services.notifications', []).service('ImpacNotifications', ["$log", "Pusher", "ImpacEvents", "IMPAC_EVENTS", "toastr", function($log, Pusher, ImpacEvents, IMPAC_EVENTS, toastr) {
    var EVENTS, _self;
    _self = this;
      ...
      service.kpis = {
        ...
        alerts: {
          ...
          "delete": function(alert_id) {
            return defaults.mnoHub + "/impac/alerts/" + alert_id;
          }
        }
      };
...
(function() {
  ...
  module.controller('WidgetAccountsBalanceCtrl', ["$scope", "$q", "ChartFormatterSvc", "$filter", function($scope, $q, ChartFormatterSvc, $filter) {
    ...
    settingsPromises = [$scope.orgDeferred.promise, $scope.accountBackDeferred, $scope.accountFrontDeferred, $scope.timePeriodDeferred.promise, $scope.histModeDeferred.promise, $scope.chartDeferred.promise, $scope.attachKpisDeferred.promise];
...
(function() {
  ...
  module.controller('WidgetAccountsBalanceSheetCtrl', ["$scope", "$q", "ImpacWidgetsSvc", "ImpacMainSvc", "ImpacUtilities", function($scope, $q, ImpacWidgetsSvc, ImpacMainSvc, ImpacUtilities) {
    var initDates, settingsPromises, unCollapsedSetting, w;
    w = $scope.widget;
    $scope.orgDeferred = $q.defer();
...
(function() {
  ...
  module.controller('WidgetAccountsExpenseWeightCtrl', ["$scope", "$q", "ChartFormatterSvc", function($scope, $q, ChartFormatterSvc) {
    ...
    $scope.accountFrontDeferred = $q.defer();
    $scope.chartDeferred = $q.defer();
    settingsPromises = [$scope.orgDeferred.promise, $scope.timePeriodDeferred, $scope.accountBackDeferred, $scope.accountFrontDeferred, $scope.chartDeferred.promise];
    $scope.forwardParams = {
...
(function() {
  ...
  module.controller('WidgetSalesOpportunitiesFunnelCtrl', ["$scope", "$q", "ChartFormatterSvc", "$filter", "ImpacWidgetsSvc", "ImpacDashboardsSvc", function($scope, $q, ChartFormatterSvc, $filter, ImpacWidgetsSvc, ImpacDashboardsSvc) {
    var getFilteredTotal, getOrderedAssigneeIds, hasOneOpportunity, selectedStatusSetting, settingsPromises, w;
    w = $scope.widget;
...
(function() {
  ...
  module.directive('settingAttachKpis', ["$templateCache", "ImpacWidgetsSvc", "ImpacKpisSvc", function($templateCache, ImpacWidgetsSvc, ImpacKpisSvc) {
    return {
      ...
      controller: ["$scope", function($scope) {
        var formatAttachedKpiTitle, settings, w;
        w = $scope.parentWidget;
        settings = {};
        ...
        $scope.attachKpi = function() {
          ...
          return ImpacKpisSvc.create('impac', $scope.kpi.endpoint, $scope.elementWatched, params).then(function(kpi) {
            ...
            return ImpacKpisSvc.show(kpi).then(function() {
              return formatAttachedKpiTitle(kpi);
            });
          });
        };
        ...
        formatAttachedKpiTitle = function(kpi) {
          return $scope.kpiFormattedTitles[kpi.id] = ImpacKpisSvc.formatKpiTarget(kpi.targets[$scope.elementWatched][0], kpi.data[$scope.elementWatched].unit, $scope.possibleTargets);
        };
        $scope.possibleTargets = [
          {
            label: 'over',
        ...
        ImpacKpisSvc.getAttachableKpis($scope.widgetEngine).then(function(kpiTemplates) {
          ...
          return $scope.elementWatched = $scope.kpi.watchables[0];
        });
        $scope.kpiFormattedTitles = {};
        _.forEach($scope.attachedKpis, function(kpi) {
          return ImpacKpisSvc.show(kpi).then(function() {
            return formatAttachedKpiTitle(kpi);
          });
        });
...
(function() {
  ...
  module.directive('settingTimePeriod', ["$templateCache", "$q", "$log", "$timeout", "ImpacTheming", function($templateCache, $q, $log, $timeout, ImpacTheming) {
    return {
      ...
      link: function(scope) {
        var getPeriod, getSetting, getUsedSettingKey, initPeriod, initUsedSetting, resetPreset, settingsPromises, updateFromDate, updateTimeRangePeriod, w;
        w = scope.parentWidget;
