...
(function () {
'use strict';
angular.module('impac.services.chart-formatter', []).service('ChartFormatterSvc', ["ImpacTheming", "$filter", "$window", "$document", function(ImpacTheming, $filter, $window, $document) {
...
module.controller('WidgetAccountsCashSummaryCtrl', ["$scope", "$q", "ChartFormatterSvc", "$filter", "ImpacWidgetsSvc", function($scope, $q, ChartFormatterSvc, $filter, ImpacWidgetsSvc) {
  ...
  $scope.getVarianceClassColor = function(element, aVariance) {
  ...
  $scope.drawTrigger = $q.defer();
  w.format = function() {
    var all_values_are_positive, chartData, data, dates, inputData, options;
...
module.controller('WidgetSalesSummaryCtrl', ["$scope", "$q", "ChartFormatterSvc", function($scope, $q, ChartFormatterSvc) {
  ...
  w.format = function() {
    ...
    if ($scope.isDataFound) {
      ...
      if ($scope.filter.value.toLowerCase().indexOf('quantity') >= 0) {
