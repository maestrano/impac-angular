...
(function () {console.info("Impac! Front-End Library - v1.3.13"); window["impac"] = {"version": "1.3.13"};}).call();
(function() {
...
(function() {
  ...
  module.controller('ImpacDashboardCtrl', ["$scope", "$http", "$q", "$filter", "$modal", "$log", "$timeout", "$window", "$templateCache", "MsgBus", "ImpacUtilities", "ImpacAssets", "ImpacTheming", "ImpacRoutes", "ImpacMainSvc", "ImpacDashboardsSvc", "ImpacWidgetsSvc", "ImpacDeveloper", function($scope, $http, $q, $filter, $modal, $log, $timeout, $window, $templateCache, MsgBus, ImpacUtilities, ImpacAssets, ImpacTheming, ImpacRoutes, ImpacMainSvc, ImpacDashboardsSvc, ImpacWidgetsSvc, ImpacDeveloper) {
    ...
    return {
      ...
      controller: ["$scope", "$timeout", "$log", function($scope, $timeout, $log) {
        ...
        $scope.removeKpi = function(kpiId) {
          return _.remove($scope.kpis, function(kpi) {
            return kpi.id === kpiId;
          });
        };
  ...
  angular.module('impac.components.kpi', []).directive('impacKpi', ["$log", "ImpacKpisSvc", function($log, ImpacKpisSvc) {
    return {
      ...
      controller: ["$scope", function($scope) {
        ...
        if (!$scope.kpi["static"]) {
          ImpacKpisSvc.show($scope.kpi).then(function(renderedKpi) {
...
(function() {
  ...
  module.controller('ImpacWidgetCtrl', ["$scope", "$log", "$q", "$timeout", "ImpacWidgetsSvc", "ImpacDashboardsSvc", function($scope, $log, $q, $timeout, ImpacWidgetsSvc, ImpacDashboardsSvc) {
    ...
    ImpacDashboardsSvc.pdfModeEnabled().then(null, null, function() {
      if (!angular.isDefined(w.expanded)) {
        w.initialWidth = w.width;
        w.width = 12;
      }
      $scope.pdfMode = true;
      ...
      return $scope.widget.hasEditAbility = false;
    });
      ...
      if (!_self.loadLocked) {
        ...
        if (needConfigurationLoad() || force) {
          ImpacMainSvc.load(force).then(function(success) {
            ...
            $http.get(ImpacRoutes.dashboards.index(orgId)).then(function(dashboards) {
...
(function() {
  angular.module('impac.services.kpis', []).service('ImpacKpisSvc', ["$log", "$http", "$filter", "$q", "$timeout", "ImpacRoutes", "ImpacMainSvc", "ImpacDashboardsSvc", "ImpacDeveloper", function($log, $http, $filter, $q, $timeout, ImpacRoutes, ImpacMainSvc, ImpacDashboardsSvc, ImpacDeveloper) {
        ...
        return $q.all([ImpacMainSvc.loadUserData(force), ImpacDashboardsSvc.load(force)]).then(function(results) {
          ...
          if (_.isEmpty(_self.getKpisTemplates()) || force) {
            ...
            return $q.all(promises).then(function(response) {
              ...
              return $log.error('Impac! - KpisSvc: Cannot retrieve kpis templates list', error);
            })["finally"](function() {
    ...
    this.update = function(kpi, params) {
      ...
      return _self.load().then(filtered_params = {}, params.name != null ? filtered_params.name = params.name : void 0, params.metadata != null ? filtered_params.settings = params.metadata : void 0, params.targets != null ? filtered_params.targets = params.targets : void 0, params.extra_params != null ? filtered_params.extra_params = params.extra_params : void 0, url = ImpacRoutes.kpis.update(_self.getCurrentDashboard().id, kpi.id), !_.isEmpty(filtered_params) ? $http.put(url, {
        kpi: params
      }).then(function(success) {
        angular.extend(kpi, success.data);
        _self.show(kpi);
        return $q.resolve(kpi);
      }, function(err) {
    ...
    this["delete"] = function(kpi) {
      var url;
      return _self.load().then(url = ImpacRoutes.kpis["delete"](_self.getCurrentDashboard().id, kpi.id), $http["delete"](url));
    };
    return this;
  }]);
...
(function() {
  angular.module('impac.services.main', []).service('ImpacMainSvc', ["$q", "$log", "$timeout", "ImpacLinking", function($q, $log, $timeout, ImpacLinking) {
...
(function() {
  ...
  angular.module('impac.services.widgets', []).service('ImpacWidgetsSvc', ["$q", "$http", "$log", "$timeout", "ImpacRoutes", "ImpacMainSvc", "ImpacDashboardsSvc", "ImpacDeveloper", function($q, $http, $log, $timeout, ImpacRoutes, ImpacMainSvc, ImpacDashboardsSvc, ImpacDeveloper) {
...
(function() {
  ...
  module.directive('dashboardSettingSyncApps', ["$templateCache", "$log", "$http", "$filter", "$modal", "$document", "ImpacMainSvc", "ImpacRoutes", "ImpacWidgetsSvc", "ImpacTheming", "poller", "$timeout", "$sce", function($templateCache, $log, $http, $filter, $modal, $document, ImpacMainSvc, ImpacRoutes, ImpacWidgetsSvc, ImpacTheming, poller, $timeout, $sce) {
    return {
      restrict: 'A',
      scope: {},
      template: $templateCache.get('dashboard-settings/sync-apps.tmpl.html'),
      link: function(scope, element, attrs) {
        var determineSyncStatus, getOffset, processAppInstancesSync, refreshDashboard, syncAppsThemingConfig;
        ...
        processAppInstancesSync = function(responseData) {
          ...
          if (!(scope.isSyncing || scope.connectors.length < 1)) {
            if (!(_.isEqual(scope.previousConnector, scope.connectors[0]) && scope.connectors[0].status === 'SUCCESS')) {
        ...
        determineSyncStatus = function(force) {
          var descriptor;
          if (force == null) {
            force = false;
          }
          descriptor = syncAppsThemingConfig.productDescriptor;
          if (_.every(scope.connectors, {
            'status': 'SUCCESS'
          }) || force) {
            return scope.syncStatus = descriptor + " is synced!";
          } else if (_.some(scope.connectors, 'last_sync_date')) {
            return scope.syncStatus = descriptor + " is partially synced.";
          } else {
            return scope.syncStatus = descriptor + " is not synced.";
          }
        };
        ...
        scope.noErrors = function() {
          return _.isEmpty(scope.failedConnectors) && _.isEmpty(scope.disconnectedConnectors) && _.isEmpty(scope.disabledConnectors);
        };
        scope.formatStatus = function(connector, modalDisplay) {
          ...
          if (connector.last_sync_date) {
            ...
            switch (connector.status) {
              case 'SUCCESS':
                status = modalDisplay ? "Synced at " + date : "is synced - " + date;
                break;
              case 'FAILED':
                status = modalDisplay ? "Previous sync " + date : "is not synced - previous sync " + date;
                break;
              case 'DISCONNECTED':
                status = modalDisplay ? "Previous sync " + date : "is not connected - previous sync " + date;
                break;
              case 'DISABLED':
                status = modalDisplay ? "Previous sync " + date : "synchronisation is disabled - previous sync " + date;
            }
          } else {
            switch (connector.status) {
              case 'FAILED':
                status = modalDisplay ? "Sync failed" : "is not synced - sync failed";
                break;
              case 'NOT SYNCED':
                status = modalDisplay ? "Never synced" : "is not synced - never synced";
                break;
              case 'DISCONNECTED':
                status = modalDisplay ? "Not connected" : "is not synced - not connected";
                break;
              case 'DISABLED':
                status = modalDisplay ? "Synchronization disabled" : "synchronisation is disabled";
            }
          }
          status = modalDisplay ? "" + status : "<strong>" + name + "</strong> " + status;
        ...
        scope.triggerSyncAlertsModal = function() {
          ...
          return modalInstance = $modal.open({
            ...
            controller: ["$scope", "connectors", "methods", function($scope, connectors, methods) {
              ...
              $scope.expandListItemOnClick = function(e) {
                var list, listItem;
                ...
                list.animate({
                  scrollTop: listItem.offset().top - list.offset().top
                }, 500);
              };
            ...
            resolve: {
              connectors: function() {
                return {
                  ...
                  disabled: scope.disabledConnectors,
                  failed: scope.failedConnectors,
                  successful: scope.successfulConnectors
                };
              },
              methods: function() {
                return {
...
(function() {
  ...
  module.controller('WidgetAccountsCustomCalculationCtrl', ["$scope", "$timeout", "$modal", "$q", "$templateCache", "$filter", "ImpacWidgetsSvc", "ChartFormatterSvc", function($scope, $timeout, $modal, $q, $templateCache, $filter, ImpacWidgetsSvc, ChartFormatterSvc) {
    var settingsPromises, w;
    w = $scope.widget;
    $scope.orgDeferred = $q.defer();
    $scope.accountsListDeferred = $q.defer();
    ...
    $scope.histModeDeferred = $q.defer();
    $scope.chartDeferred = $q.defer();
    settingsPromises = [$scope.orgDeferred.promise, $scope.accountsListDeferred.promise, $scope.formulaDeferred.promise, $scope.timePeriodDeferred, $scope.histModeDeferred.promise, $scope.chartDeferred.promise];
    w.initContext = function() {
      $scope.movedAccount = {};
    ...
    $scope.formulaModal.proceed = function() {
      ...
      return ImpacWidgetsSvc.updateWidgetSettings(w, true).then(function() {
        $scope.initSettings();
        return w.format();
      });
    };
    ...
    $scope.formulaModal.saveTimeRange = function() {
      return ImpacWidgetsSvc.updateWidgetSettings(w, true).then(function() {
        return $scope.initSettings();
      });
    };
    ...
    $scope.drawTrigger = $q.defer();
    w.format = function() {
      var all_values_are_positive, chartData, dates, lineData, lineOptions, period;
      if ($scope.isDataFound && w.isHistoryMode) {
        ...
        all_values_are_positive = true;
        angular.forEach(w.selectedAccounts, function(acc) {
          return angular.forEach(acc.balances, function(balance) {
            return all_values_are_positive && (all_values_are_positive = balance >= 0);
          });
        });
        lineData = [
          {
...
(function() {
  ...
  module.controller('WidgetAccountsProfitAndLossCtrl', ["$scope", "$q", "ChartFormatterSvc", "$filter", "ImpacWidgetsSvc", function($scope, $q, ChartFormatterSvc, $filter, ImpacWidgetsSvc) {
    ...
    setAmountDisplayed();
    w.initContext = function() {
      var firstDate, foundElem, i, j, lastDate, len, len1, ref, ref1, sElem, statement;
      if ($scope.isDataFound = angular.isDefined(w.content) && !_.isEmpty(w.content.summary) && !_.isEmpty(w.content.dates)) {
        $scope.dates = w.content.dates;
        $scope.unCollapsed = w.metadata.unCollapsed || [];
...
(function() {
  ...
  module.controller('WidgetInvoicesAgedPayablesReceivablesCtrl', ["$scope", "$q", "$log", "$filter", "ChartFormatterSvc", "ImpacWidgetsSvc", function($scope, $q, $log, $filter, ChartFormatterSvc, ImpacWidgetsSvc) {
    var selectedElementsSetting, settingsPromises, sortBy, sortData, unCollapsedSetting, w;
    w = $scope.widget;
    $scope.orgDeferred = $q.defer();
    $scope.timePeriodDeferred = $q.defer();
    $scope.widthDeferred = $q.defer();
...
(function() {
  ...
  module.controller('WidgetSalesLeadsFunnelCtrl', ["$scope", "$q", "ChartFormatterSvc", "$filter", "ImpacWidgetsSvc", "ImpacDashboardsSvc", function($scope, $q, ChartFormatterSvc, $filter, ImpacWidgetsSvc, ImpacDashboardsSvc) {
...
(function() {
  ...
  module.directive('commonDataNotFound', ["$templateCache", "$log", "$http", "ImpacAssets", "ImpacTheming", "ImpacMainSvc", function($templateCache, $log, $http, ImpacAssets, ImpacTheming, ImpacMainSvc) {
    return {
      ...
      link: function(scope, element) {
        var baseDir, dir, displayDefaultImage, hasMyobEssentialsOnly, image, imagePath, usingDefaults;
        ...
        image.onerror = function() {
          ...
          missingImageLocationMsg = usingDefaults ? 'library defaults' : scope.widgetEngine;
        ...
        if (scope.widgetEngine && baseDir.length > 0) {
          ...
          $http.get(imagePath).then(function() {
            return image.src = imagePath;
          }, function() {
  ...
  module.directive('commonTimePeriodInfo', ["$templateCache", "ImpacUtilities", function($templateCache, ImpacUtilities) {
    return {
      ...
      link: function(scope, element) {
        ...
        return scope.getDateInfo = function() {
          var context, dates;
          context = scope.context;
...
(function() {
  ...
  module.directive('settingDatesPicker', ["$templateCache", "$filter", "ImpacWidgetsSvc", "$timeout", "$compile", function($templateCache, $filter, ImpacWidgetsSvc, $timeout, $compile) {
    return {
      ...
      link: function(scope, element) {
        var applyHtml, fromDateHtml, isToToday, setting, templatesContainer, toDateHtml, w;
        w = scope.parentWidget;
        setting = {};
        ...
        fromDateHtml = "<button class=\"btn btn-sm btn-default date-button\" ng-click=\"calendarFrom.toggle()\" datepicker-popup ng-model=\"calendarFrom.value\" is-open=\"calendarFrom.opened\" ng-change=\"showApplyButton()\" min-date=\"minDate\" max-date=\"calendarTo.value\" ng-focus=\"onUse()\" ATTRS>\n  {{ calendarFrom.value | date : 'yyyy-MM-dd' }}\n</button>";
        toDateHtml = "<button class=\"btn btn-sm btn-default date-button\" ng-click=\"calendarTo.toggle()\" datepicker-popup ng-model=\"calendarTo.value\" is-open=\"calendarTo.opened\" ng-change=\"showApplyButton()\" min-date=\"calendarFrom.value\" ng-focus=\"onUse()\" ATTRS>\n  {{ calendarTo.value | date : 'yyyy-MM-dd' }}\n</button>";
...
(function() {
  ...
  module.controller('SettingFormulaCtrl', ["$scope", "$filter", "$timeout", "$log", function($scope, $filter, $timeout, $log) {
    var applyFormula, applyFormulas, calculationResults, evaluate, formatResult, getFormula, initialFormulas, interpolateAllAccountsBalances, interpolateAllAccountsNames, interpolateIndexesInFormula, isResultValid, setting, w;
    w = $scope.parentWidget;
    setting = {};
    setting.key = "formula";
    setting.isInitialized = false;
    setting.initialize = function() {
      w.formula = ((w.metadata != null) && w.metadata.formula) || "";
      evaluate(w.formula);
      return setting.isInitialized = true;
    };
    setting.toMetadata = function() {
    ...
    $scope.$watch(getFormula, function(formula) {
      return evaluate(formula);
    });
    initialFormulas = function(formulaString, arraySample) {
      var formulasArray;
      return formulasArray = _.map(arraySample, function(e) {
        return formulaString;
      });
    };
    interpolateIndexesInFormula = function(formulaString, replacement, accountIndex) {
      var balancePattern;
      balancePattern = "\\{" + accountIndex + "\\}";
      return formulaString.replace(new RegExp(balancePattern, 'g'), " " + replacement + " ");
    };
    interpolateAllAccountsBalances = function(formulasArray, accounts, accountIndex) {
      var balanceIndex, newFormulasArray;
      if (accountIndex == null) {
        accountIndex = 1;
      }
      if (!(accountIndex > accounts.length)) {
        balanceIndex = 0;
        newFormulasArray = _.map(formulasArray, function(formulaString) {
          var balance;
          balance = accounts[accountIndex - 1].balances[balanceIndex];
          balanceIndex++;
          return interpolateIndexesInFormula(formulaString, balance, accountIndex);
        });
        return interpolateAllAccountsBalances(newFormulasArray, accounts, accountIndex + 1);
      } else {
        return formulasArray;
      }
    };
    interpolateAllAccountsNames = function(formulaString, accounts, accountIndex) {
      var name, newFormula;
      if (accountIndex == null) {
        accountIndex = 1;
      }
      if (!(accountIndex > accounts.length)) {
        ...
        return interpolateAllAccountsNames(newFormula, accounts, accountIndex + 1);
      } else {
        return formulaString;
      }
    };
    applyFormulas = function(formulasArray) {
      return _.map(formulasArray, function(formula) {
        return applyFormula(formula);
      });
    };
    applyFormula = function(formula) {
      var authorized_regex, e, error;
      authorized_regex = new RegExp("^(\\{|\\d|\\}|\\/|\\+|-|\\*|\\(|\\)|\\s|\\.)*$");
      if (!formula.match(authorized_regex)) {
        return false;
      }
      try {
    ...
    calculationResults = function(formulasString, arraySample, accounts) {
      var init, interpolated;
      init = initialFormulas(formulasString, arraySample);
      interpolated = interpolateAllAccountsBalances(init, accounts);
      return applyFormulas(interpolated);
    };
    isResultValid = function(result) {
      return result && isFinite(result);
    };
    formatResult = function(formulaString, result, currency) {
      if (!isResultValid(result)) {
        return "invalid expression";
      } else if (formulaString.match(/\//) || !currency) {
        return result;
      } else {
        return $filter('mnoCurrency')(result, currency);
      }
    };
    evaluate = function(formula) {
      var accounts, currency, currentResult;
      if (!(_.isEmpty(formula) || _.isEmpty(w.content.dates))) {
        ...
        return w.isFormulaCorrect = isResultValid(currentResult);
      } else {
      ...
      link: function(scope) {
        ...
        ImpacMainSvc.load().then(function(config) {
          if ((config != null) && (config.currentOrganization != null) && parseInt(config.currentOrganization.financial_year_end_month)) {
            return fyEndMonth = parseInt(config.currentOrganization.financial_year_end_month);
          }
        })["finally"](function() {
