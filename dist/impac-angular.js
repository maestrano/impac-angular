...
(function () {console.info("Impac! Front-End Library - v1.6.1"); window["impac"] = {"version": "1.6.1"};}).call();
(function() {
  ...
  angular.module('impac.services.highcharts-factory', []).factory('HighchartsFactory', ["$filter", function($filter) {
    ...
    return Chart = (function() {
      ...
      Chart.prototype.addThresholds = function(options) {
        var data, i, j, k, len, len1, len2, ref, ref1, s, threshold, vector;
        if (options == null) {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          s = ref[i];
          if ((s != null) && _.includes(s.name.toLowerCase(), 'threshold')) {
            s.remove();
          }
        }
  ...
  module.component('chartThreshold', {
    ...
    controller: ["$timeout", "$log", "ImpacKpisSvc", "ImpacUtilities", "toastr", function($timeout, $log, ImpacKpisSvc, ImpacUtilities, toastr) {
      ...
      ctrl.saveKpi = function() {
        ...
        promise = ctrl.isEditingKpi ? ImpacKpisSvc.update(getKpi(), params, false) : (params.metadata.hist_parameters = ctrl.widget.metadata.hist_parameters, params.widget_id = ctrl.widget.id, ImpacKpisSvc.create('impac', ctrl.kpi.endpoint, ctrl.kpi.watchables[0], params));
      ...
      onChartNotify = function(chart) {
        ...
        thresholdSeries = _.filter(chart.series, function(s) {
          return _.includes(s.name.toLowerCase(), 'threshold');
        });
...
(function() {
  ...
  module.controller('WidgetAccountsCashProjectionCtrl', ["$scope", "$q", "$filter", "ImpacKpisSvc", "ImpacAssets", "HighchartsFactory", "BoltResources", function($scope, $q, $filter, ImpacKpisSvc, ImpacAssets, HighchartsFactory, BoltResources) {
    var dateFilter, getPeriod, getThresholds, legendFormatter, onClickBar, settingsPromises, w;
    w = $scope.widget;
    $scope.orgDeferred = $q.defer();
    $scope.datesPickerDeferred = $q.defer();
    $scope.intervalsOffsetsDeferred = $q.defer();
    $scope.currentOffsetsDeferred = $q.defer();
    settingsPromises = [$scope.orgDeferred.promise, $scope.datesPickerDeferred.promise, $scope.intervalsOffsetsDeferred.promise, $scope.currentOffsetsDeferred.promise];
    ...
    legendFormatter = function() {
      ...
      return "<img src='" + imgSrc + "'><br>  " + series.name;
    };
    w.format = function() {
      var options;
      options = {
