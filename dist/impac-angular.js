...
angular.module("impac.components.templates", []).run(["$templateCache", function($templateCache) {$templateCache.put("chart/chart.tmpl.html","<canvas id=\"myChart\"></canvas>");
...
(function () {
'use strict';
angular.module('impac.components.chart', []).directive('dhbChart', ["$templateCache", "$compile", "$timeout", "$log", function($templateCache, $compile, $timeout, $log) {
  return {
    restrict: 'A',
    scope: {
      widgetRendered: '=',
    ...
    link: function(scope, elem, attr) {
      ...
      scope.widgetRendered.then(function(success) {
        return $log.warn('chart promise has been resolved: use notify instead');
      }, function(error) {
        return $log.error(error);
      }, function(notifiedData) {
        return scope.draw(notifiedData);
      });
...
module.controller('WidgetAccountsAccountingValuesCtrl', ["$scope", "$q", "ChartFormatterSvc", function($scope, $q, ChartFormatterSvc) {
  ...
  w.format = function() {
    ...
    if ($scope.isDataFound) {
      ...
      return $scope.formatted.notify(data);
    }
  };
