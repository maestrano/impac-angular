    ...
    link: function(scope, elements, attrs) {
      ImpacMainSvc.load().then(function(mainConfig) {
        return ImpacDashboardsSvc.load().then(function() {
          scope.currentDhb = ImpacDashboardsSvc.getCurrentDashboard();
          scope.currency || (scope.currency = scope.currentDhb.currency || mainConfig.currentOrganization.currency || 'USD');
          scope.currencies = mainConfig.currencies;
          if (scope.currentDhb.currency == null) {
            $log.info("Dashboard " + scope.currentDhb.id + " and its widgets have been auto-configured with currency " + scope.currency);
            return scope.massAssignCurrency();
          }
        });
      });
