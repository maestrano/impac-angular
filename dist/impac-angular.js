...
angular.module('impac.services.analytics', []).factory('DhbAnalyticsSvc', ["$http", "$q", "$window", "$timeout", "$log", "ImpacLinking", "ImpacRoutes", function($http, $q, $window, $timeout, $log, ImpacLinking, ImpacRoutes) {
  ...
  ImpacLinking.getOrganizations().then(function(success) {
    ...
    return service.config.organizationId = (_.first(service.organizations)).id;
  });
...
angular.module('impac.services.theming', []).provider('ImpacTheming', function() {
  ...
  provider.configureDhbSelector = function(configOptions) {
    return angular.extend(options.dhbSelectorConfig, configOptions);
  };
  _$get = function() {
    var service;
    service = this;
    ...
    service.getDhbSelectorConfig = function() {
